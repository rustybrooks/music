...
class App(Tk):
    ...
    dials = [
        ...
        },
        {
            ...
            "mode": MODE_DIVISION,
            "property": "division",
            "list": torso_sequencer.divisions,
        },
        {
            "row": 0,
            "col": 4,
        ...
        },
        {
            ...
            "alt_mode": MODE_STYLE,
            "alt_property": "style",
            "alt_list": torso_sequencer.styles,
        },
        {
            "row": 1,
            "col": 3,
            ...
            "alt_property": "phrase",
            "alt_list": torso_sequencer.phrases,
        },
        {
            "row": 1,
            "col": 4,
            ...
            "alt_list": torso_sequencer.accent_curves,
        },
        {
            "row": 1,
            "col": 5,
        ...
        for bank, bank_file in enumerate(bank_files):
            ...
            for pattern, data in enumerate(patterns):
                ...
                for i, track in enumerate(tracks):
                    ...
                    track["slices"] = [
                        torso_sequencer.TrackSlice(**s) for s in track.pop("slices", [])
                    ]
    ...
    def dial_callback(self, degrees):
        if self.pattern is None or self.bank is None or self.track is None:
            print(
                f"need pattern or bank - bank={self.bank} pattern={self.pattern} track={self.track}"
            )
            return
    ...
    def button_release_repeat(self, row, col, bank, event):
        bkey = (row, col, bank)
        self.has_prev_key_release[bkey] = self.after_idle(
            lambda: self.button_release(row, col, bank, event)
        )
        ...
        if bank == 0:
            if self.mode == MODE_TRACKS:
                ...
                track = self.torso.get_track(
                    (self.bank, self.pattern, row * self.cols + col)
                )
    ...
    def get_value(
        self, dial=None, interpolate=None, asint=False, asindex=True, control=None
    ):
        track = self.torso.get_track((self.bank, self.pattern, self.track))
    ...
    def interpolate_set_value(self, value, interpolate=None):
        ...
        if not self.control and "list" in dial:
            ...
            if self.control:
                fmin, fmax, ftype = (
                    dial.get("alt_min", dial["min"]),
                    dial.get("alt_max", dial["max"]),
                    dial.get("alt_type", dial["type"]),
                )
            else:
        ...
        if self.mode in [MODE_TRACKS, MODE_MUTE]:
            for row in range(2):
                for col in range(8):
                    index = row * self.cols + col
                    track = self.torso.get_track(
                        (self.bank, self.pattern, index), create=False
                    )
                    if index == self.track:
                        color = "active"
                ...
                self.w_buttons[0][index].configure(
                    bg=self.colors[
                        ...
                        if not n
                            ...
                            "black_active" if this_n in our_notes else "black_inactive"
                        )
                    ],
                    text="".join([str(x) for x in this_n]),
                )
...
if __name__ == "__main__":
    ...
    app = App(options.bank_files, config_file=options.config)
    app.mainloop()
    app.torso.join()
